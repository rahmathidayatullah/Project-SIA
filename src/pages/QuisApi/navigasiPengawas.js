...
import { changeCurrentIndex, fetchQuiz } from "features/QuizApi/action";
export default function NavigasiPengawas() {
  let dispatch = useDispatch();
  const navigasi = useSelector((state) => state.quizApi.allData);
  let dataQuiz = useSelector((state) => state.quizApi);
  useEffect(() => {
    dispatch(fetchQuiz());
